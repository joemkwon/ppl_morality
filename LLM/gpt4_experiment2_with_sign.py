...
def run_trial(trial, context):
    ...
    image_path = os.path.join(SCRIPT_DIR, 'experiment_stimuli', 'exp2_stimuli_with_sign', trial['image'])
    ...
    for goal in trial['goals']:
        ...
        results.append({
            "image": trial['image'],  # Explicitly include the image filename
            "goal": goal,
            "question": question,
            "responses": responses
        })
    ...
    for condition in tqdm(experiment_data['conditions'], desc="Processing conditions"):
        ...
        for trial in tqdm(condition['trials'], desc=f"Processing trials for {condition['name']}"):
            ...
            condition_results.append({
                "image": trial['image'],  # Explicitly include the image filename at this level too
...
def main():
    ...
    parser.add_argument(
        ...
        type=str, 
        nargs='?', 
        default=os.path.join(SCRIPT_DIR, "experiment_stimuli", "exp2_stimuli_with_sign", "experiment_config.json"), 
    ...
    parser.add_argument(
        ...
        type=str, 
        nargs='?', 
        default=os.path.join(SCRIPT_DIR, "experiment_results", "gpt4_experiment2_with_sign", "experiment_results.json"), 

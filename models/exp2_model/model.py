...
def universalized_plan_utility(gridworld, my_goal_utility, my_grass_shortcutiness, goal_probabilities, grass_capacity, grass_community_cost):
    total_grass_steps = 0
...
def generate_forward_model_table(gridworld, trajectory, my_goal_utility, goal_proportions, scenario_name, my_goal_name, num_bins=100, num_agents=100, num_simulations=10, grass_capacity=200, grass_community_cost=1000000):
...
if __name__ == "__main__":
    # Flag to activate parameter sweep
    perform_parameter_sweep = True  # Set to True to activate the sweep
    ...
    # Define the grass capacity and community cost values for the sweep
    def get_sweep_parameters():
        if perform_parameter_sweep:
            grass_capacities = list(range(100, 300, 100))  
            grass_community_costs = [100, 2000, 4000, 6000, 8000, 10000]
            return grass_capacities, grass_community_costs
        else:
            return [default_grass_capacity], [1000000]  # Default values

    grass_capacities, grass_community_costs = get_sweep_parameters()
    ...
    # Initialize data structure to collect results for plotting
    ...
    # Loop over grass_capacity and grass_community_cost values
    for grass_capacity in grass_capacities:
        for grass_community_cost in grass_community_costs:
            print(f"\nRunning with grass_capacity: {grass_capacity}, grass_community_cost: {grass_community_cost}")

            # Initialize data structure for this parameter combination
            results_for_plotting[(grass_capacity, grass_community_cost)] = {}

            # Prepare data structure to collect data for the CSV
            csv_data = []
            ...
            for scenario_name, goal_proportions in goal_proportions_scenarios:
                ...
                results_for_plotting[(grass_capacity, grass_community_cost)][scenario_name] = {}
                ...
                for current_goal in goals_to_analyze:
                    ...
                    for file_name in file_names:
                        ...
                        for idx, goal_utility in enumerate(sampled_utilities):
                            ...
                            if metadata_key not in scenario_tables:
                        ...
                        # Compute mean myopia
                        mean_myopia = np.sum(np.array(myopia_values) * np.array(posterior_probabilities))

                        # Calculate 'moral judgment'
                        moral_judgment = 100 - (mean_myopia * 100)

                        # Extract steps on grass from file name
                        base_name = os.path.basename(file_name)  # e.g., 'map1_1.json'
                        steps_on_grass = int(base_name.split('_')[-1].split('.')[0]) - 1  # Extract number after '_', subtract 1

                        # Collect data for the CSV
                        csv_data.append({
                            'Goal': current_goal,
                            'Steps on grass': steps_on_grass,
                            'myopia': mean_myopia,
                            'moral judgment': moral_judgment
                        })
                    ...
                    results_for_plotting[(grass_capacity, grass_community_cost)][scenario_name][current_goal] = posterior_distributions

            # Save the collected data to a CSV file for the current parameter combination
            df_csv = pd.DataFrame(csv_data)
            csv_file_name = f'myopia_moral_judgment_grass_capacity_{grass_capacity}_community_cost_{grass_community_cost}.csv'
            df_csv.to_csv(csv_file_name, index=False)
            print(f"\nSaved CSV data to '{csv_file_name}'")

    # Now, create the plots
    print("\nCreating summary plots...")
    for grass_capacity, grass_community_cost in itertools.product(grass_capacities, grass_community_costs):
        for scenario_name in results_for_plotting[(grass_capacity, grass_community_cost)]:
            plt.figure(figsize=(12, 6))
            plt.title(f'Inference Results - Grass Capacity {grass_capacity}, Community Cost {grass_community_cost}, Scenario {scenario_name}')
            plt.xlabel('Map')
            ...
            for idx_map, map_name in enumerate(x_ticks):
                for current_goal in goals_to_analyze:
                    posterior_distributions = results_for_plotting[(grass_capacity, grass_community_cost)][scenario_name][current_goal]
            ...
            # Save the plot
            output_dir = os.path.join('summary_plots', f'grass_capacity_{grass_capacity}_community_cost_{grass_community_cost}', scenario_name)
            os.makedirs(output_dir, exist_ok=True)
            output_path = os.path.join(output_dir, f'summary_plot_{scenario_name}_grass_{grass_capacity}_cost_{grass_community_cost}.png')

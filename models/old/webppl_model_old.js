...
// Sample the total number of agents to simulate
var numAgents = Math.round(gaussian(200, 25));

// Function to sample a goal for an agent
var sample_goal_for_agent = function() {
    return categorical([
      { value: 'hospital', weight: 1 },
      { value: 'busStop', weight: 1 },
      { value: 'coffeeShop', weight: 1 }
    ]);
  };
  
  // Sample utilities for each agent
  var sample_utility_for_goal = function(goal) {
    switch (goal) {
...
// Global counter for total grass traffic
var totalGrassTraffic = 0;

// Increment the grass traffic counter
var increment_grass_traffic = function(locationType) {
    if (locationType === 'G') totalGrassTraffic += 1;
};

// Global counter for total utility
var totalUtility = 0;

// Increment the total utility counter
var increment_total_utility = function(agentUtility) {
    totalUtility += agentUtility;
};

// Simulation function to simulate all agents
var simulate_agents = function(numAgents) {
    for (var i = 0; i < numAgents; i++) {
        var agentId = 'agent' + i;
        var goal = sample_goal_for_agent();
        var agentUtility = sample_utility_for_goal(goal);
        var trajectory = optimal_trajectory(agentId, gridworld, initial_x, initial_y);
        
        // Increment grass traffic for each step on grass
        trajectory.forEach(function(location) {
            increment_grass_traffic(location);
        });
        
        // Increment total utility when the agent reaches its goal
        increment_total_utility(agentUtility);
    }
    return { totalGrassTraffic: totalGrassTraffic, totalUtility: totalUtility };
  };
  
  // Run the simulation
  var results = simulate_agents(numAgents);
  results;

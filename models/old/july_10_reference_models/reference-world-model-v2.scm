...
;; Basically in this version, the 4 diagonal movements are added, restaurant open is assumed, goal restaraunts have unique utility distributions, and the agent is always walking (no biking option)
...
(define directions (list 'west 'east 'north 'south 'north-west 'north-east 'south-west 'south-east))

;; Define available actions combining motions and directions
(define available_actions (mem (lambda (agent-id) (cons (pair 'stay 'stay) directions))))

;; Define utility constants for different restaurants
(define SUSHI_MEAN_UTILITY 1000)
(define PIZZA_MEAN_UTILITY 100)
(define VEGETARIAN_MEAN_UTILITY 10)
(define SUSHI_UTILITY_VARIANCE 100)
(define PIZZA_UTILITY_VARIANCE 10)
(define VEGETARIAN_UTILITY_VARIANCE 1)

;; Define the utility of restaurants
(define restaurant_utility (mem (lambda (agent-id restaurant_type)
  (case restaurant_type
    (('sushi) (gaussian SUSHI_MEAN_UTILITY SUSHI_UTILITY_VARIANCE))
    (('pizza) (gaussian PIZZA_MEAN_UTILITY PIZZA_UTILITY_VARIANCE))
    (('vegetarian) (gaussian VEGETARIAN_MEAN_UTILITY VEGETARIAN_UTILITY_VARIANCE))
    (else 0)
))))
...
(define food_utility (mem (lambda (agent-id location_type)
  (case location_type
      ...
      (('danner) 0)
      (('office) 0)
      (else (restaurant_utility agent-id location_type))
))))
...
(define utility_function (mem (lambda (agent-id gridworld state_x state_y action)
  ...
  (let ((state_motion_utility (motion_utility agent-id location_type action)))
...
(define trajectory_from_initial_state (mem (lambda (agent-id gridworld state_x state_y)
     ...
     (let ((next_state_x (second next_state)))
     (let ((next_state_y (third next state)))

...
def save_trajectories_with_simulation_data(agents_data, gridworld, simulation_parameters, agent_type, file_name="trajectories_policies.jsonl"):
    ...
    data = {
        ...
        "agent_type": agent_type  # Adding agent type information to the data
    }
...
def simulate(
    ...
    agent_type="main",  # Parameter to distinguish agent types
    seed=None
):
    ...
    for agent_id in agent_ids:
        ...
        # Determine if this agent is rule-following or rule-exceptional based on the type
    ...
    # Save all collected data to JSON, including the agent type
    save_trajectories_with_simulation_data(agents_data, gridworld, simulation_parameters, agent_type)

    # Write results to a single txt file with a '~' delineator between agent types
    with open("combined_simulation_results.txt", "a") as file:
        ...
        file.write("~" * 40 + "\n\n")  # '~' delineator between agent types
...
def run_simulation_for_all_agents(
    ...
    # Loop over each agent type and run the simulation
    for agent_type in agent_types:
        ...
        # Collect results for this agent type
        simulation_result = {
    ...
    # Write combined results for all agent types to the text file
    with open("combined_simulation_results.txt", "a") as file:
        file.write(f"Random Seed: {seed}\n" if seed is not None else "Random Seed: None\n")
        file.write(f"Max Iterations: {MAX_ITERATIONS}\n")
        file.write(f"Number of agents: {num_agents}\n")
        ...
        file.write("-" * 40 + "\n\n")  # '-' delineator after all three agent types

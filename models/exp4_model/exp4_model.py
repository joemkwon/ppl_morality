...
def universalized_plan_utility(gridworld, my_goal_utility, my_grass_shortcutiness, goal_probabilities):
    ...
    for agent_id in range(population):
        ...
        total_utility -= int(grass_steps) # sep 19 addition
...
def parameter_sweep(gridworld, goal_utility, grass_shortcutiness, goal_probabilities,
    ...
    for grass_cap in range(*grass_capacity_range):
        for pop in range(*population_range):
            ...
            utility = universalized_plan_utility(gridworld, goal_utility, grass_shortcutiness, goal_probabilities)
...
def generate_forward_model_table(gridworld, trajectory, my_goal_utility, goal_proportions, scenario_name, num_bins=100, num_agents=100, num_simulations=10, grass_capacity=200):
    ...
    for myopia in tqdm(myopia_values, desc=f"Generating table with forward model for {scenario_name}"):
        for shortcut in shortcut_values:
            ...
            universalized_utilities = [universalized_plan_utility(gridworld, my_goal_utility, shortcut, goal_probabilities) for _ in range(num_simulations)]
...
if __name__ == "__main__":
    ...
    num_goal_utility_simulations = 1  # Number of times to sample goal utility and then average over
    grass_capacity = 1  
    population = 100

    num_bins = 50
    num_simulations = 10 # This refers to the number of simulations in the universalization step
    num_agents = 100
    ...
    for _ in range(num_goal_utility_simulations):
        ...
        goal_utility = 65 # temporarily set to mean of distribution while we are finding good grass cap params
        ...
        for file_name in file_names:
            ...
            for scenario_name, goal_proportions in goal_proportions_scenarios:
                ...
                if metadata_key in scenario_tables:
                    ...
                    new_table = generate_forward_model_table(gridworld, trajectory, goal_utility, goal_proportions, scenario_name, num_bins=num_bins, num_agents=num_agents, num_simulations=num_simulations, grass_capacity=grass_capacity)
                ...
                myopia_values, posterior_probabilities = infer_myopia_parameter(trajectory, {metadata_key: forward_model_table})

                # Collect data
                key = (file_name, scenario_name)
                if key not in collected_data:
                    collected_data[key] = []
                collected_data[key].append((myopia_values, posterior_probabilities))

    # Calculate average goal utility
    avg_goal_utility = total_goal_utility / num_goal_utility_simulations

    # Process collected data and plot
    for (file_name, scenario_name), data_list in collected_data.items():
        avg_myopia_values = np.mean([d[0] for d in data_list], axis=0)
        avg_posterior_probabilities = np.mean([d[1] for d in data_list], axis=0)

        # Plot and save the averaged results
        plot_myopia_inference(avg_myopia_values, avg_posterior_probabilities, file_name, avg_goal_utility, scenario_name, grass_capacity)